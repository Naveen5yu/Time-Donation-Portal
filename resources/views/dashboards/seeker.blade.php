...
<body>
  ...
  <!-- Dashboard Cards -->
  <div class="dashboard">
    <a href="{{ route('seeker.time_requests.index') }}" class="card card-my-requests">
      My Requests
      <span class="tooltip">View your time requests</span>
    </a>
    <a href="{{ route('seeker.time_requests.create') }}" class="card card-create-request">
      Create Request
      <span class="tooltip">Create a new time request</span>
    </a>
    <a href="{{ route('seeker.donors.index') }}" class="card card-donors">
      Donors List
      <span class="tooltip">Browse available donors</span>
    </a>
  </div>
  ...
  <div class="chat-list">
    ...
    <div id="chat-items">
      @if ($requests->whereNotNull('donor_id')->isEmpty())
          <p class="no-chats">No active chats available.</p>
      @else
          @foreach ($requests->whereNotNull('donor_id') as $request)
              <a href="{{ route('seeker.chat', ['timeRequest' => $request->id]) }}" 
                 class="chat-item" 
                 data-title="{{ strtolower($request->title) }}" 
                 data-status="active"
                 data-latest-message="{{ $request->messages->isNotEmpty() ? $request->messages->first()->message : '' }}"
                 data-sender="{{ $request->messages->isNotEmpty() ? ($request->messages->first()->sender_name ?? 'Unknown') : 'Unknown' }}">
                  ...
                  @if ($request->messages->isNotEmpty())
                      ...
                      <span class="tooltip">From: {{ $request->messages->isNotEmpty() ? ($request->messages->first()->sender_name ?? 'Unknown') : 'Unknown' }}<br>{{ Str::limit($request->messages->isNotEmpty() ? $request->messages->first()->message : 'No messages yet', 50) }}</span>
                  @else
